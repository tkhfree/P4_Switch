...
#[ extern void exact_delete_promote  (int tableid, uint8_t* key, uint8_t* value);
#[ extern void lpm_delete_promote    (int tableid, uint8_t* key, uint8_t depth, uint8_t* value);
#[ extern void ternary_delete_promote(int tableid, uint8_t* key, uint8_t* mask, uint8_t* value);

#[ extern void exact_lookup_promote  (int tableid, uint8_t* key, uint8_t* value);
#[ extern void lpm_lookup_promote    (int tableid, uint8_t* key, uint8_t depth, uint8_t* value);
...
for table in hlir16_tables_with_keys:
    ...
    if table.match_type == "LPM":
        ...
        #[ for(c = ${byte_idx-1}, d = 0; c >= 0; c--, d++) *(reverse_buffer+d) = *(key+c);
        #[ for(c = 0; c < ${byte_idx}; c++) *(key+c) = *(reverse_buffer+c);
        #[ lpm_delete_promote(TABLE_${table.name}, (uint8_t*)key, prefix_length, (uint8_t*)&action);

    if table.match_type == "EXACT":
        #[ printf("DDDDDDDEEEEEEEEEELLLLLLLLLLEEEEEEEEEEEEETTTTTTTTTTTEEEEEEEEEEEE");
        #[ exact_delete_promote(TABLE_${table.name}, (uint8_t*)key, (uint8_t*)&action);
...
for table in hlir16_tables_with_keys:
    ...
    if table.match_type == "LPM":
        ...
        #[ for(c = ${byte_idx-1}, d = 0; c >= 0; c--, d++) *(reverse_buffer+d) = *(key+c);
        #[ for(c = 0; c < ${byte_idx}; c++) *(key+c) = *(reverse_buffer+c);
        #[ lpm_lookup_promote(TABLE_${table.name}, (uint8_t*)key, prefix_length, (uint8_t*)&action);

    if table.match_type == "EXACT":
        #[ exact_lookup_promote(TABLE_${table.name}, (uint8_t*)key, (uint8_t*)&action);
...
for table in hlir16_tables_with_keys:
    ...
    for action in table.actions:
        # TODO is there a more appropriate source for this than the annotation?
        action_name_str = get_action_name_str(action)
        #{ if(strcmp("$action_name_str", ctrl_m->action_name)==0) {//"$action_name_str"
	#[     printf("strcmppp-----");
        #[     struct ${table.name}_action action;
        #[     action.action_id = action_${action.action_object.name};
        #[     debug("From controller: remove entry to $$[table]{table.name} with action $$[action]{action.action_object.name}\n");
        for j, p in enumerate(action.action_object.parameters.parameters):
            #[ uint8_t* ${p.name} = (uint8_t*)((struct p4_action_parameter*)ctrl_m->action_params[$j])->bitmap;
    ...
    valid_actions = ", ".join(["\" T4LIT(" + get_action_name_str(a) + ",action) \"" for a in table.actions])
    #[ debug(" $$[warning]{}{!!!! Table remove entry} on table $$[table]{table.name}: action name $$[warning]{}{mismatch}: $$[action]{}{%s}, expected one of ($valid_actions).\n", ctrl_m->action_name);
    #} }
...
for table in hlir16_tables_with_keys:
    ...
    for action in table.actions:
        ...
        #[     struct ${table.name}_action action;
        #[     action.action_id = action_${action.action_object.name};
        #[     debug("From controller: get  entry to $$[table]{table.name} with action $$[action]{action.action_object.name}\n");
        for j, p in enumerate(action.action_object.parameters.parameters):
            #[ uint8_t* ${p.name} = (uint8_t*)((struct p4_action_parameter*)ctrl_m->action_params[$j])->bitmap;
    ...
    valid_actions = ", ".join(["\" T4LIT(" + get_action_name_str(a) + ",action) \"" for a in table.actions])
    #[ debug(" $$[warning]{}{!!!! Table get entry} on table $$[table]{table.name}: action name $$[warning]{}{mismatch}: $$[action]{}{%s}, expected one of ($valid_actions).\n", ctrl_m->action_name);
    #} }
...
for table in hlir16_tables_with_keys:
    ...
    #[     return;
    #} }
#[     debug(" $$[warning]{}{!!!! Table remove entry}: table name $$[warning]{}{mismatch} ($$[table]{}{%s}), expected one of ($keyed_table_names).\n", ctrl_m->table_name);
#} }
...
for table in hlir16_tables_with_keys:
    ...
    #[     return;
    #} }
#[     debug(" $$[warning]{}{!!!! Table get entries}: table name $$[warning]{}{mismatch} ($$[table]{}{%s}), expected one of ($keyed_table_names).\n", ctrl_m->table_name);
#} }
...
#[     uint8_t port = (uint8_t*)(ctrl_m->portid);
