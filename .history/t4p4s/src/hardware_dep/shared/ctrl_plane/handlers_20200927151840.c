...
int handle_p4_msg(char* buffer, int length, p4_msg_callback cb)
{
	...
	switch (header->type)
	{
		...
		case P4T_CTRL_INITIALIZED:
			...
			cb(&ctrl_m);
			break;
		case P4T_REMOVE_TABLE_ENTRY:
			rval = handle_p4_remove_table_entry(netconv_p4_remove_table_entry((struct p4_remove_table_entry*)buffer), &ctrl_m);
#ifdef T4P4S_DEBUG
			if (rval != 0) {
				printf("[CTRL]    :: REMOVE_TABLE_ENTRY rval=%d\n", rval);
			}
#endif
			if (rval<0) return rval;
			cb(&ctrl_m);
			break;
		case P4T_GET_TABLE_ENTRIES:
			rval = handle_p4_get_table_entries(netconv_p4_get_table_entries((struct p4_get_table_entries*)buffer), &ctrl_m);
#ifdef T4P4S_DEBUG
			if (rval != 0) {
				printf("[CTRL]    :: GET_TABLE_ENTRIES rval=%d\n", rval);
			}
#endif
			if (rval<0) return rval;
			cb(&ctrl_m);
...
int handle_p4_remove_table_entry(struct p4_remove_table_entry* m, struct p4_ctrl_msg* ctrl_m)
{
        int i;
        int num_params;
	int size;
	...
	buffer = (char*)(m) + sizeof(struct p4_remove_table_entry);
	offset = 0;
...
int handle_p4_get_table_entries(struct p4_get_table_entries* m, struct p4_ctrl_msg* ctrl_m)
{
        int i;
        int num_params;
		int size;
	...
	buffer = (char*)(m) + sizeof(struct p4_get_table_entries);
	offset = 0;
