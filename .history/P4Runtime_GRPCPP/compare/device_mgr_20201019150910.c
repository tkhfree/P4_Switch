...
grpc::Status table_delete(device_mgr_t *dm, const ::p4::v1::TableEntry &table_entry) {
	...
	element_t *elem = get_element(&(dm->id_map), table_id);cr
	argument_t *arg = NULL;
	...
	for(const auto &match: table_entry.match()) {
		field_id = match.field_id();
		arg = get_argument(elem,set_field_id);
		if(arg==NULL) {
			printf("NULL ARGUMENT for FIELD_ID=%d\n",set_field_id);
		}
		switch (match.field_match_type_case())
...
grpc::Status dev_mgr_packet(device_mgr_t *dm, const ::p4::v1::StreamMessageRequest &request, ::p4::v1::StreamMessageResponse *response) {
	grpc::Status status = grpc::Status::OK;
	...
	for(const auto &data : request.packetout().metadata()) {
		...
		const auto metadata_id  = data.medadata_id();
		status = controller_packet_out(device_mgr_t *dm, &data, payload);
		}
	return status;
