...
struct p4_remove_table_entry* create_p4_remove_table_entry(char* buffer, uint16_t offset, uint16_t maxlength) {
	struct p4_remove_table_entry* remove_table_entry;
        if (offset+sizeof(struct p4_remove_table_entry) >= maxlength) return 0; /* buffer overflow */
	remove_table_entry = (struct p4_remove_table_entry*)(buffer + offset);
	remove_table_entry->header.length = sizeof (struct p4_remove_table_entry);
	remove_table_entry->header.type = P4T_REMOVE_TABLE_ENTRY;
	remove_table_entry->read_size = 0;
	remove_table_entry->table_name[0] = '\0';
	return remove_table_entry;
}

inline struct p4_remove_table_entry* netconv_p4_remove_table_entry(struct p4_remove_table_entry* m) {
	return m; /*nothing to do*/
}

struct p4_get_table_entries* create_p4_get_table_entries(char* buffer, uint16_t offset, uint16_t maxlength) {
	struct p4_get_table_entries* get_table_entries;
        
	return get_table_entries;
}

inline struct p4_get_table_entries* netconv_p4_get_table_entries(struct p4_get_table_entries* m) {
	return m; /*nothing to do*/
}
