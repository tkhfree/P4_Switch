...
for table in hlir16_tables_with_keys:
    #{ void ${table.name}_modify_table_entry(struct p4_ctrl_msg* ctrl_m) {
    for i, k in enumerate(table.key.keyElements):
        # TODO should properly handle specials (isValid etc.)
        if k.get_attr('header') is None:
            if k.header_name!='meta':
    ...
    for action in table.actions:
        ...
        #[     struct ${table.name}_action action;
        #[     action.action_id = action_${action.action_object.name};
        #[     debug("From controller: modify old entry to $$[table]{table.name} with action $$[action]{action.action_object.name}\n");
        for j, p in enumerate(action.action_object.parameters.parameters):
            #[ uint8_t* ${p.name} = (uint8_t*)((struct p4_action_parameter*)ctrl_m->action_params[$j])->bitmap;
    ...
    valid_actions = ", ".join(["\" T4LIT(" + get_action_name_str(a) + ",action) \"" for a in table.actions])
    #[ debug(" $$[warning]{}{!!!! Table modify entry} on table $$[table]{table.name}: action name $$[warning]{}{mismatch}: $$[action]{}{%s}, expected one of ($valid_actions).\n", ctrl_m->action_name);
    #} }
...
#{ void ctrl_modify_table_entry(struct p4_ctrl_msg* ctrl_m) {
for table in hlir16_tables_with_keys:
	#{ if (strcmp("${table.name}", ctrl_m->table_name) == 0) {
	#[     ${table.name}_modify_table_entry(ctrl_m);
	#[     return;
	#} }
#[     debug(" $$[warning]{}{!!!! Table modify entry}: table name $$[warning]{}{mismatch} ($$[table]{}{%s}),expected one of ($keyed_table_names).\n", ctrl_m->table_name);
#} }
...
#[         ctrl_initialized();
#[     } else if (ctrl_m->type == P4T_MODIFY_TABLE_ENTRY) {
#[         ctrl_modify_table_entry(ctrl_m);
#}     }
#} }
